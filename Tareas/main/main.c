...
#define TIEMPO_TASK1 500
#define TIEMPO_TASK2 1000
...
void motor(void *pvParameters)
{
    while (1)
    {
        ...
        if (adc_raw < 2048)
        {
            ...
            gpio_set_level(AIN2, 0); // Establece AIN2 en alto 
            duty = (int)((adc_raw/2048.0)*255);
         }
        if (adc_raw > 2048)
        {
             gpio_set_level(AIN1, 0); // Establece AIN1 en alto
             gpio_set_level(AIN2, 1); // Establece AIN2 en alto   
             duty = (int)((adc_raw*(255/2048) -255));
...
void app_main(void) // Función principal de la aplicación
{
    ...
    xTaskCreatePinnedToCore(adctask, "Tarea ADC", 2048, NULL, 1, NULL, 0);
    xTaskCreatePinnedToCore(motor, "Tarea motor", 2048, NULL, 1, NULL, 1);
